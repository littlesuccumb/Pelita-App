                    ...
                    <div class="relative p-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="flex items-baseline gap-2">
                                <h3 class="text-4xl font-black text-white">{{ $stats['total_barang'] ?? 0 }}</h3>
                    ...
                    <form method="GET" action="{{ route('admin.barang.index') }}">
                        <div class="grid grid-cols-1 lg:grid-cols-5 gap-5 mb-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select name="status" class="block w-full px-4 py-3.5 pr-10 bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 focus:border-blue-500 dark:focus:border-blue-400 focus:bg-white dark:focus:bg-gray-600 transition-all text-sm text-gray-900 dark:text-gray-100 appearance-none cursor-pointer">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select name="kondisi" class="block w-full px-4 py-3.5 pr-10 bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-xl focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 focus:border-blue-500 dark:focus:border-blue-400 focus:bg-white dark:focus:bg-gray-600 transition-all text-sm text-gray-900 dark:text-gray-100 appearance-none cursor-pointer">
...
{{-- Script Notifikasi untuk Success --}}
@if(session('success'))
<script>
    document.addEventListener('DOMContentLoaded', function() {
        console.log('%c✅ Success notification triggered', 'color: #10b981; font-weight: bold;');
        console.log('Session Data:', {
            success: '{{ session("success") }}',
            action: '{{ session("action", "") }}',
            barang_name: '{{ session("barang_name", "") }}',
            barang_code: '{{ session("barang_code", "") }}',
            barang_jumlah: {{ session('barang_jumlah', 0) }},
            barang_kategori: '{{ session("barang_kategori", "") }}',
            foto_count: {{ session('foto_count', 0) }}
        });
        
        // ✅ CEK APAKAH NotificationSystem ADA
        if (typeof NotificationSystem === 'undefined') {
            console.error('%c❌ NotificationSystem is not defined!', 'color: #ef4444; font-weight: bold;');
            alert('{{ session("success") }}');
            return;
        }
        
        const action = '{{ session("action", "") }}';
        console.log('%cAction: ' + action, 'color: #3b82f6; font-weight: bold;');
        
        try {
            if (action === 'created') {
                console.log('%c🎯 Calling dataAdded...', 'color: #8b5cf6; font-weight: bold;');
                
                NotificationSystem.dataAdded(
                    '{{ session("barang_name", "Barang") }}',
                    '{{ session("barang_code", "-") }}',
                    {{ session('barang_jumlah', 0) }},
                    {{ session('foto_count', 0) }},
                    '{{ session("barang_kategori", "") }}'
                );
                
                console.log('%c✅ dataAdded called successfully', 'color: #10b981; font-weight: bold;');
                
            } else if (action === 'updated') {
                console.log('%c🎯 Calling dataUpdated...', 'color: #8b5cf6; font-weight: bold;');
                
                NotificationSystem.dataUpdated(
                    '{{ session("barang_name", "Barang") }}',
                    [{ icon: '<i class="fas fa-check"></i>', text: 'Data berhasil diperbarui' }]
                );
                
                console.log('%c✅ dataUpdated called successfully', 'color: #10b981; font-weight: bold;');
                
            } else if (action === 'deleted') {
                console.log('%c🎯 Calling deleted...', 'color: #8b5cf6; font-weight: bold;');
                
                NotificationSystem.deleted(
                    '{{ session("barang_name", "Barang") }}',
                    '{{ session("barang_code", "-") }}'
                );
                
                console.log('%c✅ deleted called successfully', 'color: #10b981; font-weight: bold;');
                
            } else if (action === 'maintenance_scheduled') {
                console.log('%c🎯 Calling maintenanceScheduled...', 'color: #8b5cf6; font-weight: bold;');
                
                NotificationSystem.maintenanceScheduled(
                    '{{ session("barang_name", "Barang") }}',
                    {{ session('jumlah', 0) }},
                    '{{ session("teknisi", "") }}',
                    '{{ session("tanggal", "") }}'
                ...
                console.log('%c✅ maintenanceScheduled called successfully', 'color: #10b981; font-weight: bold;');
                
            } else if (action === 'import_completed') {
                console.log('%c🎯 Calling importCompleted...', 'color: #8b5cf6; font-weight: bold;');
                
                NotificationSystem.importCompleted({{ session('success_count', 0) }}, 0);
                
                console.log('%c✅ importCompleted called successfully', 'color: #10b981; font-weight: bold;');
                
            } else if (action === 'partial_import_fail') {
                console.log('%c🎯 Calling partial import...', 'color: #8b5cf6; font-weight: bold;');
                
                @if(session('error_details'))
                    const errorDetails = {!! json_encode(collect(session('error_details'))->map(function($err) {
                        return ['icon' => '<i class="fas fa-times"></i>', 'text' => $err];
                    })->toArray()) !!};
                    
                    NotificationSystem.importCompleted(
                ...
                console.log('%c✅ partial import called successfully', 'color: #10b981; font-weight: bold;');
                
            } else {
                // Default success notification
                console.log('%c🎯 Using default success notification', 'color: #8b5cf6; font-weight: bold;');
                console.log('No action specified, action value:', action);
                
                NotificationSystem.success('basic', {
                    message: '{{ session("success") }}'
                });
                
                console.log('%c✅ default success called', 'color: #10b981; font-weight: bold;');
            }
        } catch(error) {
            console.error('%c❌ Error in notification:', 'color: #ef4444; font-weight: bold;', error);
            alert('{{ session("success") }}'); // Fallback
        }
    });
</script>
@endif

{{-- Script Notifikasi untuk Error --}}
@if(session('error'))
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if(session('validation_errors'))
            ...
            NotificationSystem.validationError(validationErrors);
            
        @elseif(session('action') === 'import_failed')  {{-- ✅ TAMBAH session() --}}
            // Notifikasi untuk import failed
            NotificationSystem.error('import-failed', {
                message: '{{ session("error") }}',
                @if(session('error_details'))
                details: {!! json_encode(collect(session('error_details'))->map(function($err) {
                    return ['icon' => '<i class="fas fa-times"></i>', 'text' => $err];
                })->toArray()) !!}
                @endif
            });
            
        @elseif(session('action') === 'delete_failed')  {{-- ✅ TAMBAH session() --}}
            // Notifikasi untuk delete failed
            NotificationSystem.error('delete-failed', {
                message: '{{ session("error") }}',
                details: [
                    { icon: '<i class="fas fa-exclamation-triangle"></i>', text: 'Barang: {{ session("barang_name", "") }}' }
                ]
            });
            
        @else
            // Default error notification
            NotificationSystem.error('basic', {
                message: '{{ session("error") }}'
            });
        @endif
    });
...
{{-- Script Notifikasi untuk Warning --}}
@if(session('warning'))
<script>
    document.addEventListener('DOMContentLoaded', function() {
        console.log('⚠️ Warning notification triggered');
        
        @if(session('warning_details'))
            // Warning dengan detail
            const warningDetails = {!! json_encode(collect(session('warning_details'))->map(function($err) {
                return ['icon' => '<i class="fas fa-exclamation"></i>', 'text' => $err];
            })->toArray()) !!};
            
            console.log('Warning Details:', warningDetails);
            
            NotificationSystem.warning('partial-upload-fail', {
                message: '{{ session("warning") }}',
                details: warningDetails
            });
        @else
            // Default warning notification
            NotificationSystem.warning('basic', {
                message: '{{ session("warning") }}'
            });
        @endif
    });
...
{{-- Script Notifikasi untuk Info --}}
@if(session('info'))
<script>
    document.addEventListener('DOMContentLoaded', function() {
        console.log('ℹ️ Info notification triggered');
        
        NotificationSystem.info('basic', {
